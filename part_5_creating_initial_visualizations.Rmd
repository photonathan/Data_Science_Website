...
Create a a table that we can use to create a plot that matches the MnDOT plot. 

# Connect to MySQL Database

```{r echo = FALSE, message = FALSE, warning= FALSE, cache=TRUE, include=TRUE}
library(ggplot2)
library(dplyr)
library(odbc)
library(reshape2)
...
The MnDOT report combined the 'In' and 'Out' directions for each day, so I'll start by doing that.

In MySQL, I'll create a new view in that takes the sum of both the 'I' and 'O' values and group them by 'date' and 'bikeometer_id'.
