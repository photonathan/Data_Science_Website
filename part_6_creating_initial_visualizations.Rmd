...
```{r}
df.long %>% ggplot(aes(month_day, counts, group = year, color = year)) + geom_line() + geom_area(aes(fill = year, group = year), alpha = 0.5, position = 'identity')
```

```{r}
ggplot(df.long, aes(x = month_day)) +
  geom_area(stat = 'bin', binwidth = 1, position = "identity", alpha = .5)
```

```{r}
line <- ggplot(df.long, (aes(x=month_day, y=counts, fill=year, group=year, color = year))) + geom_density(alpha = 0.2)
line
```
