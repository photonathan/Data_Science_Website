...
Right now, the table with both 'In' and 'Out' *Direction* looks like this:
...
For those not familiar with MySQL views, they look like tables but are actually recalculated each time they are accessed. This is what we want so the view is automatically updated to include the new data as our Python program imports the new Bike Arlington data into my 'counts_daily' MySQL table.  Thus, we don't need to change our Python program to generate new and useful tables in MySQL.
...
**Alternatively**, instead of using a MySQL statement, you can use the R package *dplyr* to pull the data from MySQL.
```{r}
counts_daily_total <- counts_daily %>% 
  group_by(date) %>% 
  summarise(count = sum(count), bikeometer_id=bikeometer_id, is_weekend = is_weekend, 
            month = month, day = day, year = year, month_day = month_day) %>% collect()
counts_daily_total
```
```{r}
counts_daily_total <- counts_daily %>% 
  group_by(date, bikeometer_id) %>% summarise() %>% collect()
counts_daily_total
```
...
In both cases you can see the 'I' and 'O' directions are combined!

Next, we will filter for the dates similar to those chosen by the MnDOT. We will also look at 
...
```{r}
counts_daily_total %>% select(bikeometer_id=c(14,15,16,18,22,31,39), year=c(2017, 2018, 2019))
```


```{sql connection=con}
CREATE OR REPLACE VIEW vw_filtered_counts_daily_total_2017_to_2019 AS 
SELECT * FROM bikeometers_db.counts_daily_total 
WHERE bikeometer_id IN (14,15,16,18,22,31,39) 
AND Year(Date) in (2017,2018,2019);
```

```{r cache=TRUE}
filtered_counts_daily_total_2017_to_2019 <- tbl(con, 'vw_filtered_counts_daily_total_2017_to_2019')
