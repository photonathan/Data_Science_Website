...
In a previous post, I chose a time-frame and which Bikeometers were best to plot. I'll outline how to filter using *dplyr* or MySQL. 
## dplyr

```{r}
counts_daily_total %>% filter(between(year, 2017, 2019) & bikeometer_id %in% c(14,15,16,18,22,31,39))
```
### Future note to self
Originally, class(counts_daily_total$year,month,day == 'S3 value Bigint' which doesn't play nicely with dplyr. Be sure to convert number columns into *int* before applying filters.
