...
type(root)
```
Our object 'root' is now an 'xml.etree.ElementTree.Element' object that we can iterate over.
```{python}
list(root)
```
...
Iterating over it, we can see there are a lot of "Element 'counter'..." objects, each representing a different Bikeometer with details of its own. 

We can use item.getchildren() to see the child elements of each Bikeometer.
...
We can see each Bikeometer has children: name, description, trail_id, trail_name, latitude, longitude, and region.

For each Bikeometer, I'll put it
