...
# Goal
Query the API and return the details of each Bikeometer and the number of bikers in a date range.
...
Our object 'root' is now an 'xml.etree.ElementTree.Element' object that we can iterate over with a *for loop* or we can use the *getchildren* function.
...
However, we get a **DepricationWarning** that we should use *list(elm)* instead.

```{python}
list(root)
```

Using the *list* function we can see there are a lot of "Element 'counter'..." objects, each representing a different Bikeometer with details of its own.
...
```
Takeing a look inside our bikeometer_details object, we see a list of tuples, where each tuple is a seperate Bikeometer. 
