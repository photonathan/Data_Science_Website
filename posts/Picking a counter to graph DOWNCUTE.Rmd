...
Generating these plots returned multiple errors similar to this: 'Removed 32 rows containing missing values (geom_point)'.

# Fixing the Errors

I wanted to look at the table to see what the exactly looked like. So, I defined a SQL Select statement to filter by my desired date range and Bikeometer ID 28 (Rosslyn Bikeometer). The 'ORDER BY count desc' sorts the table by counts in descending order.
...
```{r}
sql_cmd <- "SELECT * FROM counts.counts_daily WHERE bikeometer_id = 28 AND date >= '2017-03-12' AND date <= '2017-05-15' ORDER BY count desc"
...
I used the same function but adjusted the scale of the y-axis to 1200.
