...
def all_counts_by_date_to_sql():
    ...
    columns = ('counter_id', 'date', 'direction', 'count', 'is_weekend')
...
def all_counts_by_hour_to_sql():
    ...
    ***Each sentance will be it's own function***
    ...
    # Hardcoded the counterID list to not stress the server
        ...
        if end_date >= date.today():
            end_date = date.today() - timedelta(days=1)    
    columns = ('counter_id', 'date', 'direction', 'count', 'hour', 'is_weekend')
    df = pd.DataFrame(count_in_date_range_list, columns=columns)
    # Replaces table counts, use if_exists='append' to insert new values into the table
...
def bikeometer_to_sql():
    ...
    # Create the empty list that will include [(Name, counterID, Lat, Long, Region, RegionID)(...)]
    bikeometer_details = []
    ...
    columns = ('counter_id', 'name', 'latitude', 'longitude', 'region', 'region_id')
    df = pd.DataFrame(data=bikeometer_details, columns = columns)
    df[["counter_id", "name", "latitude", "longitude", "region", 'region_id']] = df[["counter_id", "name", "latitude", "longitude", "region", 'regionID']].astype('string')
    ...
    if interval == 'd':
        for type_tag in tree.findall('count'):
            ...
            if date.weekday() <= 4:
                ...
                is_weekend = 1
            single_tuple = (counter_id, date, direction, count, is_weekend)
    ...
    if interval == 'h':
        for type_tag in tree.findall('count'):
            ...
            if date.weekday() <= 4:
                ...
                is_weekend = 1
            single_tuple = (counter_id, date, direction, count, hour, is_weekend)
            count_in_date_range_list.append(single_tuple)
    return count_in_date_range_list


def all_counts_parameter():
    start_date = date(year=2011, month=7, day=1)
    end_date = date(year=2012, month=7, day=1)


def new_counts_parameter():
    pass

def all_counts_api_call(parameter_object, cleaning_object, ):
...
def dataframe_to_sql(dataframe, schema_name, table_name, if_exists_behavior):
    ...
    '''
    engine = create_engine(f'mysql+mysqldb://{input("Enter username: ")}:{getpass("Enter password: ")}@localhost/{schema_name}', echo=False)   
    # Replaces table counts, use if_exists='append' to insert new values into the table
    df.to_sql(str(table_name), con=engine, index=False, if_exists=str(if_exists_behavior))
    # Reads the table
    #df2 = pd.read_sql('counters', con=engine)
...
# DataframeToSql(dataframe = df, 
#                schema_name = 'counts', 
#                table_name = 'test', 
#                if_exists_behavior = 'fail')

# Can change interval to 'h' and put hour into list to get hourly numbers
...
def first_sql_date():
    ...
    engine = create_engine(f'mysql+mysqldb://{input("Enter username: ")}:{getpass("Enter password: ")}@localhost/counts', echo=False)
    with engine.connect() as con:
        date = con.execute('SELECT MIN(Date) FROM counts')
        for day in date:
            date = day[0]
    return date
